...
namespace Villermen.RuneScapeCacheTools.Cache
{
    ...
    public class ExtendableExtensionGuesser : IExtensionGuesser
    {
        ...
        private string GuessExtensionsAction(byte[] fileData)
        {
            ...
            if (DataHasMagicNumber(fileData, new byte[] { 0x47, 0x49, 0x46, 0x38, 0x37, 0x61 }) ||
                DataHasMagicNumber(fileData, new byte[] { 0x47, 0x49, 0x46, 0x38, 0x39, 0x61 }))
            {
                return "gif";
            }
            ...
            if (DataHasMagicNumber(fileData, new byte[] { 0x49, 0x49, 0x2a, 0x00 }) ||
                DataHasMagicNumber(fileData, new byte[] { 0x4d, 0x4d, 0x00, 0x2a }))
            {
                return "tiff";
            }

            // mp3 (0xfffb and ID3).
            if (DataHasMagicNumber(fileData, new byte[] { 0xff, 0xfb }) ||
                DataHasMagicNumber(fileData, new byte[] { 0x49, 0x44, 0x33 }))
            {
                return "wav";
            }
