...
namespace Villermen.RuneScapeCacheTools.File
{
    ...
    /// present in the index for the files to be listed in a <see cref="ReferenceTableFile" />.
    /// </summary>
    public class ReferenceTableFile
    {
        /// <exception cref="DecodeException"></exception>
        public static ReferenceTableFile Decode(byte[] data)
        {
        ...
        /// <summary>
        /// The files described by this <see cref="ReferenceTableFile" />.
        /// </summary>
        ...
        /// <summary>
        /// The version of this <see cref="ReferenceTableFile" />. Only available when <see cref="Format" /> >= 6.
        /// </summary>
        public int? Version { get; set; } = (int)DateTimeOffset.UtcNow.ToUnixTimeSeconds();

        /// <summary>
        /// Gets the <see cref="CacheFileInfo"/> for the given file in the index described by this
        /// <see cref="ReferenceTableFile" />. The info will be a clone and can be freely altered.
        /// </summary>
        public CacheFileInfo GetFileInfo(int fileId)
        {
            if (!this._fileInfos.ContainsKey(fileId))
            {
                throw new CacheException($"File {fileId} does not exist in this reference table.");
            }
